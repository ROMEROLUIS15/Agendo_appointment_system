    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          subtitle={summary.totalRevenue > 0 ? `Margen: ${Math.round((summary.netProfit / summary.totalRevenue) * 100)}%` : undefined}
        ...
        <div className="space-y-3">
          <div>
            <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
              <span>Ingresos</span><span>{formatCurrency(summary.totalRevenue)}</span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
              <span>Gastos</span><span>{formatCurrency(summary.totalExpenses)}</span>
            </div>
            <div className="h-3 rounded-full bg-muted overflow-hidden">
              <div className="h-full bg-red-400 rounded-full"
                style={{ width: summary.totalRevenue > 0 ? `${Math.min((summary.totalExpenses / summary.totalRevenue) * 100, 100)}%` : '0%' }} />
            </div>
          </div>
        </div>
      </Card>
              ...
              {mockTransactions.map((txn) => (
                <div key={txn.id} className="flex items-center gap-3 p-3 rounded-xl bg-surface">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-semibold text-green-600">+{formatCurrency(txn.net_amount)}</p>
                    {(txn.discount ?? 0) > 0 && (
